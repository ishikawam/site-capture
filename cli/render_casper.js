...
if (system.args.length > 4) {
    // 引数 3, engine (phantomjs or slimerjs) 4,url 5,width(デフォルト1024) 6,height(デフォルトwidth*3/4) 7, useragent(デフォルトMac) 8, zoom(デフォルト100)
...
var trim = urls[6] ? true : false; // heightが0ならtrimしない
...
casper.open(url).viewport(width, height).then(function() {
    ...
    if (user_agent.match(/iPhone/) || user_agent.match(/iPad/)) {
        ...
        zoomFactor = ViewportZoom.get(width, meta_viewport);

        console.log('zoom: ' + zoomFactor);

    }
    this.zoom(zoomFactor * zoom * 0.01);
